...
async function loadPatentData() {
  try {
    ...
    // Fetch categories - FIXED: Changed from ~/content/patents/ to ~/data/patents/
    ...
    const loadPromises = categories.value.map(async (category) => {
      try {
        // Try to import the category-specific JSON file - FIXED: Changed from ~/content/patents/ to ~/data/patents/
    ...
    if (!patentsLoaded) {
      try {
        ...
        // FIXED: Changed from ~/content/patents/ to ~/data/patents/
